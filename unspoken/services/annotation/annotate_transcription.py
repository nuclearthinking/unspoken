...
def annotate(stt_result: SpeachToTextResult, diarization_result: DiarizationResult) -> TranscriptionResult:
    ...
    for segment in stt_segments:
        ...
        relevant_segments = [s for s in sorted_diarization_segments if s.start < segment.end and s.end > segment.start]
...
def align_transcription_and_diarization(
    transcription: List[TranscriptionSegment], diarization: List[SpeakerSegment]
