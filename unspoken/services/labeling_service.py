...
class LabelingService:
    ...
    def merge_segments(self, task_id: int, segment_ids: List[int]) -> LabelingSegment:
        ...
        segments_to_merge = [
            segment for segment in labeling_task.segments
        ...
        segments_between = [
            segment for segment in labeling_task.segments
            if segment.start >= segments_to_merge[0].start and segment.end <= segments_to_merge[1].start and segment not in segments_to_merge
        ]
        if segments_between:
