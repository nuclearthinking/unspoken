...
async def upload_audio(file: UploadFile):
    ...
    file_mime_type = magic.from_buffer(file_data[:2048], mime=True)
    file_type = MimeType(file_mime_type)
    if not file_type.is_supported():
        raise HTTPException(status_code=400, detail=f'File format {file_mime_type} is not supported.')

    temp_file = db.save_temp_file(TempFile(file_name=str(uuid.uuid4()), file_type=file_type))
